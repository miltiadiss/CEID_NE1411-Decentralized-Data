...
from pyspark.sql.functions import from_json, col, avg, count, window, stddev, max, min, lit
...
# Extract the relevant fields from the station_info_stream
station_info_with_status = station_info_stream \
    .selectExpr("data.stations.station_id", 
                "data.stations.name", 
                "data.stations.lat", 
                "data.stations.lon", 
                "last_updated")  # Access last_updated from the root level

# Extract the relevant fields from the station_status_stream
station_status = station_status_stream \
    .selectExpr("data.stations.station_id", 
                "data.stations.num_bikes_available", 
                "data.stations.num_docks_available") \
    .alias("station_status")

# Join station_info_stream with station_status_stream on station_id
station_info_with_status = station_info_with_status \
    .join(station_status, "station_id") \
...
# Join the weather stream with the station info and status stream
weather_station_info = station_info_with_status \
    ...
    .select(
        ...
        "main.temp", "main.humidity", "wind.speed", "clouds.all", "precipitation", "utilization_rate"
    )
...
# Add city name to the city utilization dataframe
city_utilization = city_utilization \
    .withColumn("city_name", lit(CITY_NAME))

# Final hourly summary with required fields
